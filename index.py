    ...
    if guild_id in data:
        if channelid == data[guild_id]:
            try:
                ...
                response = chat.send_message(f"Hello, I'm {message.author.display_name}. Answer the prompt in less than 1800 character: promt: {message.content}. Your mention name is {client.user.mention} You are in a discord server named: {message.guild.name} Your owner is Afif")
